...
while ret:
    ...
    if detection_active:
        ...
        if result.multi_hand_landmarks:
            ...
            distance_thumb_index = math.hypot(fore_finger[0] - thumb[0], fore_finger[1] - thumb[1])
                    ...
                    for i in range(len(points)):
                        for j in range(1, len(points[i])):
                            if points[i][j - 1] is None or points[i][j] is None:
                                continue
                # If the forefinger is near a drawn point, erase it
                            if math.hypot(fore_finger[0] - points[i][j][0], fore_finger[1] - points[i][j][1]) < 20:
                                cv2.circle(paintWindow, points[i][j], 20, (255, 255, 255), -1)
                                points[i][j] = None  # Remove the point after erasing
